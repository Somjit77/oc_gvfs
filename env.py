...
class CollectObjects:
    def __init__(self, seed, breadth=7, length=7):
        np.random.seed(seed)
        # Environment Details
    ...
    def step(self, action):
        ...
        return {'image':self.image, 'position': self.state}, reward, self.done
...
class MiniGrid:
    def __init__(self, game, seed):
        ...
        self.env.seed(seed)
        if 'MiniGrid' in game:
            # self.env = RGBImgObsWrapper(self.env) # Get rid of the 'mission' field
    ...
    def step(self, action):
        ...
        state = np.asarray(next_obs['image']/255.0, dtype=np.float32)
        return {'image': state, 'position': (0,0)}, float(reward), done # Position not implemented
