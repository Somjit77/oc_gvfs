...
parser.add_argument("--algorithm", help='algorithm', default='dqn') #dqn, gvf, hc_gvf, gvf_esp, hc_gvf_esp
...
if 'sa' in args.algorithm:
    ...
    args.sa_decay_steps = args.sa_num_train_steps
if 'dis' in args.algorithm:
    args.discovery = True
if 'hc' in args.algorithm:
    args.hand_crafted_cumulants = True
episodes_before_transfer = args.train_episodes
load = False
...
def run_loop(agent, environment, accumulator, seed, args):
    ...
    frames = 0
    if load:
        load_dir = f'Results/logs-16.16/models/'
        checkpointer = utils.Checkpointer(load_dir+f'{args.algorithm}_run_{seed}_{2500}')
        params = checkpointer.load()
        checkpointer = utils.Checkpointer(load_dir+f'{args.algorithm}_qn_run_{seed}_{2500}')
        qn_params = checkpointer.load()
        print('Params Loaded')
        if args.use_slot_attention:
            load_model_dir = f'Results/logs-16.16/sa_model_{seed}/'
            slot_train_count = 200000
            agent.load_slot_attention_model(load_model_dir, args, new_key, None, slot_train_count)
            print('Loaded slot attention model')
            src = load_model_dir+f'{slot_train_count}'
            dst = model_dir+f'{slot_train_count}'
            shutil.copyfile(src, dst)
            args.sa_num_train_steps = 400000
        else:
            slot_train_count = 0
        environment.add_goal()
        transfer = True
    else:
        slot_train_count = 0
        transfer = False
    ...
    for episode in tqdm(range(1, args.train_episodes)):
        # Reset Env
        state = environment.reset(random=True)
        ...
        while not timestep.last():
            ...
            if frames % 1000 == 999 and slot_train_count < args.sa_num_train_steps:
                if args.use_slot_attention:
                    ...
                    train_slots(args, model_dir, seed, accumulator, 10000, transfer, total_steps)
            ...
            if accumulator.is_ready(min_samples):
                ...
                if args.discovery:
                    ...
                    if train_step % args.unroll_steps == 0:
                        qn_data = accumulator.get_multiple_samples(args.batch_size, args.discount_factor, args.unroll_steps)
                        qn_loss, qn_params, qn_learner_state, params, learner_states, dqn_learner_state, losses = agent.question_train(params, 
                        qn_params, qn_data, cumulants, dqn_learner_state, learner_states, qn_learner_state, key, train=True)
                    # Train Main Network
                    else:
                        ...
                        qn_params, data, cumulants, dqn_learner_state, learner_states, qn_learner_state, key, train=False)
                else:
                    data = accumulator.sample(args.batch_size, args.discount_factor)
    ...
    checkpointer = utils.Checkpointer(save_dir+f'{args.algorithm}_qn_run_{seed}_{episode}')
    checkpointer.save(qn_params)

def train(args):
    for run in range(1): #parallel run
        seed = args.runs + 1000
