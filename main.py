...
def run_loop(agent, environment, accumulator, seed, args):
    ...
    if args.use_slot_attention:
        ...
        # model_dir = f'Results/logs-9.19/sa_model_{seed}/'
            ...
            if accumulator.is_ready(min_samples):
                ...
                if args.discovery:
                    ...
                    if train_step % args.unroll_steps == 0:
                        # qn_data = qn_buffer.reshape(args.batch_size, args.discount_factor, args.unroll_steps)
                    ...
                    if args.hand_crafted_cumulants:
                        ...
                        cumulants = None  # np.random.uniform(low=-1.0, high=1.0, size=(args.batch_size, args.num_gvfs))
                    qn_loss, _, _, params, learner_states, dqn_learner_state, losses = agent.question_train(params,
                                                                                                            qn_params,
                                                                                                            data,
                                                                                                            cumulants,
...
def train(args):
    for run in range(1):
        ...
        epsilon_cfg = dict(
            ...
            power=1.)
        '''Pre Training'''
        # model_dir = train_slots.train(args, seed)
        agent = DQN(
